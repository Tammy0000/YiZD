...
import com.isandy.yizd.Pojo.StartCharge;
import com.isandy.yizd.dao.OrderMapper;
...
public class YiChargeContext {
    ...
    public void init(ByteBuf byteBuf) {
        try {
            ...
            redis.hset(this.StrBCD, "Seq", String.valueOf(this.int_sequence));
//            log.info("测试到这一步？");
        } catch (Exception e) {
            log.error("报文初始化排序失败");
        }
        ...
        try {
            if (DaHuaCmdEnum.远程启机命令回复.getCmd() == ByteUtils.toInt(this.TypeData)) {
                ...
                String StrData = ByteUtils.bytesToHexFun2(this.PayData);
                StrData = StrData.replace(" ", "");
                redis.hset(this.StrBCD+this.MuzzleNum, "ordercode", StrData);
                StartCharge startCharge = new StartCharge();
                startCharge.setOrdercode(StrData);
                startCharge.setLogintime(new Date());
                startCharge.setStrbcd(this.StrBCD);
                startCharge.setMuzzlenum(this.MuzzleNum);
                startCharge.setResult(this.StartMuzzleResult > 0 ? "成功" : "失败");
                orderMapper.insert(startCharge);
            }
        } catch (Exception e) {
        ...
        try {
            if (DaHuaCmdEnum.登陆.getCmd() == ByteUtils.toInt(this.TypeData)) {
                    ...
                    /*
                    注意，枪数量跟Seq直接保存在对应的BCD下面。方便后面通过查找，其余信息均保存的形式是
                    StrBCD+枪号
                    例如桩号是:1234567 枪号是1
                    则在Redis保存的形式是12345671，以此类推2号枪保存的是12345672
                     */
                    redis.hset(this.StrBCD, "SumMuzzle", String.valueOf(this.SumMuzzle));
                    for (int i = 1; i < this.SumMuzzle + 1; i++) {
                        redis.hset(this.StrBCD+i, "ChargeSeries", this.ChargeSeries);
                        redis.hset(this.StrBCD+i, "Network", this.Network);
                        redis.hset(this.StrBCD+i, "SIMCard", this.SIMCard);
                        redis.hset(this.StrBCD+i, "MuzzleNum", String.valueOf(i));
        ...
        try {
            if (DaHuaCmdEnum.心跳包Ping.getCmd() == ByteUtils.toInt(this.TypeData)) {
                ...
                redis.hset(this.StrBCD + this.MuzzleNum, "MuzzleStatus", muzzle);
            }
        } catch (Exception e) {
        ...
        try {
                ...
                if (DaHuaCmdEnum.交易记录.getCmd() == ByteUtils.toInt(this.TypeData)) {
                    this.PayData = new byte[16];
                    log.info("内容长度是"+this.message_body.length);
                    System.arraycopy(this.message_body, 0, this.PayData, 0, 16);
                    String StrData = ByteUtils.bytesToHexFun2(this.PayData).replace(" ", "");
                    byte[] muzzlenum = new byte[1];
                    ...
                    wrapper.eq("ordercode", StrData);
                    PayCharge payCharge = payMapper.selectOne(wrapper);
                    if (payCharge == null) {
                        PayCharge pays = new PayCharge();
                        pays.setStrbcd(this.StrBCD);
                        pays.setMuzzlenum(this.MuzzleNum);
                        pays.setStarttime(this.StartTime);
                        pays.setEndtime(this.EndTime);
                        pays.setStartmethod(this.StartMethod);
                        pays.setConsume(this.Pay);
                        pays.setSumele(this.SumEle);
                        pays.setPaytime(this.PayTime);
                        pays.setOrdercode(StrData);
                        payMapper.insert(pays);
                    }else {
                        UpdateWrapper<PayCharge> w1 = new UpdateWrapper<>();
                        w1.eq("ordercode", StrData)
                                .set("strbcd", this.StrBCD)
                                .set("muzzlenum", this.MuzzleNum)
                                .set("starttime", this.StartTime)
                                .set("endtime", this.EndTime)
                                .set("startmethod", this.StartMethod)
                                .set("consume", this.Pay)
                                .set("sumele", this.SumEle)
                                .set("paytime", this.PayTime)
                                .set("ordercode", StrData);
                        payMapper.update(null, w1);
                    }
                }
            } catch (Exception e) {
            ...
            try {
                ...
                if (DaHuaCmdEnum.上传实时监测数据.getCmd() == ByteUtils.toInt(this.TypeData)) {
                    ...
                    redis.hset(this.StrBCD + this.MuzzleNum, "MuzzleNum", String.valueOf(this.MuzzleNum));
                    redis.hset(this.StrBCD + this.MuzzleNum, "StrBCD", this.StrBCD);
                    ...
                    redis.hset(this.StrBCD + this.MuzzleNum, "MuzzleVolt", String.valueOf(this.MuzzleVolt));
                    byte[] ec = new byte[2];
                    ec[0] = this.message_body[29];
                    ec[1] = this.message_body[30];
                    this.MuzzleEC = (double) ByteUtils.toInt(ec, false) / 10;
                    redis.hset(this.StrBCD + this.MuzzleNum, "MuzzleEC", String.valueOf(this.MuzzleEC));
                    byte[] sum = new byte[4];
                    ...
                    redis.hset(this.StrBCD + this.MuzzleNum, "SumCharge", String.valueOf(this.SumCharge));
                    byte[] bht = new byte[1];
                    ...
                    this.Consume = this.MuzzleWork == 3 ? this.Consume : 0;
                    redis.hset(this.StrBCD + this.MuzzleNum, "Consume", String.valueOf(this.Consume));
                    byte[] HF = new byte[2];
                    ...
                    redis.hset(this.StrBCD+this.MuzzleNum, "HardwareFailure", this.HardwareFailure);
                }
            } catch (Exception e) {
